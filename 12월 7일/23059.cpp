...
vector<string> topologicalSort(set<string>& item) {
    vector<string> result; //result배열
    priority_queue<string, vector<string>, greater<>> pq, pq_temp; //우선순위 큐

    for (auto iter = item.begin(); iter != item.end(); iter++) { //아이템 확인하면서
        if (indegree.find(*iter) == indegree.end()) //진입차수가 존재하지 않는다면(0 이라면)
            pq.push(*iter); //큐에 넣는다.
    }
    while (!pq.empty()) { //비어있지않다면
        string node = pq.top(); //node에 top(위에있는 값을 넣고)
        pq.pop(); //큐에서 제거한다

        result.push_back(node); //현재 정점 순서에 삽입
        for (int i = 0; i < graph[node].size(); i++) { //그 정점에서 연결된 노드전부확인
            string next_node = graph[node][i]; //다음 노드 방문
...
int main() {
    ...
    for (int i = 0; i < result.size(); i++) //배열 크기만큼 돌면서
        cout << result[i] << '\n'; //배열 출력
